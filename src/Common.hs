...
module Common (
  ...
  DayCountConvention,
  Underlying,
  financial, scheduled, orZero, date, mkt,
  allOf, forward, adjustDateTime, modifyDate, physical, square,getAmRate, zcb
) where
...
--modifyDate :: (Day -> Day) -> DiffDateTime
modifyDate adj = DiffDateTime (\(UTCTime day tod) -> UTCTime (adj day) tod)

--modifyTime :: (DiffTime -> DiffTime) -> DiffDateTime
...
physical vol (Mkt comod unit loc) =
...
physicalWith vol (Mkt comod unit loc) (TradeAttr modifyAttrs) =
...
physicalWithDuration vol (Mkt comod unit loc) dur =
